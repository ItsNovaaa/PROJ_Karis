    ...
    $(function() {
      dataTable = $('#datatable').dataTable({
          ...
          //   {data: 'DT_RowIndex', name: 'DT_RowIndex'},
            {data: 'id_member', name: 'Nama'},
            {data: 'nama_member', name: 'kegiatan'},
            // {
    ...
    $('body').on('click','.conva', function(e){
            ...
            cancelButtonText: 'Batal',
                    ...
                    if (result.isConfirmed) {
                        // Simpan data
                        $.ajax({
                            url:'{{ route('member.storeOrUpdate') }}',
                            type:'POST',
                            headers: {
                                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                            },
...
$('body').on('click', '.conva-edit', function (e) {
      e.preventDefault();
      var id = $(this).data('id');
      var selectedValue = $('#id_member').val();
      console.log(id);
      $('#id_member').val(id);
      $(document).data('id', id);
      $.ajax({
        url:'{{ route('member.show') }}/' + id,
        type:'GET',
        headers: {
        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
        },
          success: function (response) {
              ...
              $('#nomer_telepon_member').val(response.result?.nomer_telepon_member );
      ...
      Swal.fire({
          ...
          if (result.isConfirmed) {
            var formData = $('#form').serialize();
              $.ajax({
                url:'{{ route('member.storeOrUpdate') }}/'+id,
                type: "POST",
                data: formData,
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
