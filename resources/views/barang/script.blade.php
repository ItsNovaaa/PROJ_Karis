    ...
    $(function() {
      dataTable = $('#datatable').dataTable({
          ...
          //   {data: 'DT_RowIndex', name: 'DT_RowIndex'},
            {data: 'id_barang', name: 'Nama'},
            {data: 'nama_barang', name: 'kegiatan'},
            // {
    ...
    $('body').on('click','.conva', function(e){
            ...
            cancelButtonText: 'Batal',
                    ...
                    if (result.isConfirmed) {
                        // Simpan data
                        $.ajax({
                            url:'{{ route('barang.storeOrUpdate') }}',
                            type:'POST',
                            headers: {
                                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                            },
...
$('body').on('click', '.conva-edit', function (e) {
      e.preventDefault();
      var id = $(this).data('id');
      var selectedValue = $('#id_barang').val();
      console.log(id);
      $('#id_barang').val(id);
      $(document).data('id', id);
      $.ajax({
        url:'{{ route('barang.show') }}/' + id,
        type:'GET',
        headers: {
        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
        },
          success: function (response) {
              ...
              $('#kode_barang').val(response.result?.kode_barang );
              $('#kategori_barang').val(response.result?.kategori_barang );
              $('#jumlah_barang').val(response.result?.jumlah_barang );
              $('#kategori_barang').val(response.result?.kategori_barang );
      ...
      Swal.fire({
          ...
          if (result.isConfirmed) {
            var formData = $('#form').serialize();
              $.ajax({
                url:'{{ route('barang.storeOrUpdate') }}/'+id,
                type: "POST",
                data: formData,
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
